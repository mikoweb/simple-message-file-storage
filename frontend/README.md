#  simple-message-file-storage - Frontend

## Useful links:

* [Starting an App](https://ionicframework.com/docs/developing/starting)
* [Ionic UI Components](https://ionicframework.com/docs/components)
* [Vue - Single-File Components](https://vuejs.org/guide/introduction.html#single-file-components)
* [DevExtreme - Getting Started with DataGrid](https://js.devexpress.com/Documentation/Guide/UI_Components/DataGrid/Getting_Started_with_DataGrid/)
* [Add DevExtreme to a Vue Application](https://js.devexpress.com/Documentation/Guide/Vue_Components/Add_DevExtreme_to_a_Vue_Application/)
* [Dependency Injection - TSyringe](https://github.com/microsoft/tsyringe)
* [Axios](https://axios-http.com/docs/instance)
